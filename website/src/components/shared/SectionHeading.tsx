...
export function SectionHeading({ eyebrow, title, description, action, align = 'left' }: SectionHeadingProps) {
  return (
    <div className={`mb-10 flex gap-6 ${align === 'center' ? 'flex-col items-center text-center' : 'flex-col items-start md:flex-row md:items-end md:justify-between'}`}>
      ...
      {action && (
        <Link className="inline-flex shrink-0 font-semibold text-primary underline-offset-4 hover:underline" href={action.href}>
          {action.label} →
        </Link>
      )}
    </div>
